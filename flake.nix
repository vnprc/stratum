{
  ...
  let
      ...
      run-local-pool = pkgs.writeScriptBin "run-local-pool"
        ''
        cargo -C roles/pool -Z unstable-options run -- -c roles/pool/config-examples/pool-config-local-tp-example.toml
	'';

      run-job-server = pkgs.writeScriptBin "run-job-server"
        ''
        cargo -C roles/jd-server -Z unstable-options run -- -c roles/jd-server/config-examples/jds-config-local-example.toml
	'';

      run-job-client = pkgs.writeScriptBin "run-job-client"
        ''
        cargo -C roles/jd-client -Z unstable-options run -- -c roles/jd-client/config-examples/jds-config-local-example.toml
	'';

      run-translator-proxy = pkgs.writeScriptBin "run-translator-proxy"
        ''
        cargo -C roles/translator -Z unstable-options run -- -c roles/translator/config-examples/tproxy-config-local-jdc-example.toml
	'';
